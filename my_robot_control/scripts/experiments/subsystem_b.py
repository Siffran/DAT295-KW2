...
class Subsystem_B:
    ...
    def update_wheel_turns(self, data):
        ...
        curr_y = self.pos_y_wheel_encoder
        curr_x = self.pos_x_wheel_encoder
        self.pos_y_wheel_encoder += dist * math.cos(self.direction) - curr_y
        self.pos_x_wheel_encoder += dist * math.sin(self.direction) - curr_x

    def update_wheel_turns_2d(self, data):
               # transform from quaternion to euler angles
        pose = data.pose[data.name.index('robot::wheel_1')]
        quaternion = (
            pose.orientation.x,
        ...
        if self.prev_part_circ > self.part_circ:
            ...
            self.pos_y_wheel_encoder += dist * math.cos(self.direction)
            self.pos_x_wheel_encoder += dist * math.sin(-self.direction)
...
if __name__ == '__main__':
    try:
        ...
        rospy.Subscriber("/gazebo/link_states", gazebo_msgs.msg.LinkStates, subsystem_b.update_wheel_turns_2d)
